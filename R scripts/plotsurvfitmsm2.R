...
plot.survfit.msm2 <- function (x, from = 1, to = NULL, range = NULL, covariates = "mean", 
                               interp = c("start", "midpoint"), ci = c("none", "normal", "bootstrap"), B = 100, legend.pos = NULL, 
                               ...
                               col.poly = "red", alpha.poly = 0.2, ...) # I have added these two arguments (see line 52)
{
  ...
  if (ci != "none") {
    ...
    polygon(c(times, rev(times)), 1 - c(lower, rev(upper)), col = adjustcolor(col.poly, alpha.f = alpha.poly), border=NA) # added
  }
  ...
  minfrom <- rep(which(dat$"(state)" == from & !duplicated(dat$subjstate)) - 
                   which(!duplicated(dat$"(subject)")), table(dat$"(subject)")) + 
    1
  ...
  lines(survival::survfit(Surv(survdat$survtime, survdat$died) ~ 1), 
        mark.time = mark.time, col = col.surv, lty = lty.surv, 
        lwd = lwd.surv, ...)
  timediff <- (rg[2] - rg[1])/50
