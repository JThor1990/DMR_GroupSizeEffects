...
# re-plot the sma with confidence intervals generated by bootstrapping
# create new data set at a high resolution (200 points from min to max)
preds <- data.frame(expand.grid(teethwidth.ln = seq(min(teethwidth$teethwidth.ln),
                                                    max(teethwidth$teethwidth.ln), length.out = 200)))
...
plot(log(weight) ~ log(TeethWidth), data = teethwidth, las = 1, ylab = "ln Mass, g", 
     xlab = "ln Incisor Width, mm", col = adjustcolor("grey30", alpha.f = 0.6), ylim = c(4.3, 5.3))
points(log(weight) ~ log(TeethWidth), data = filter(teethwidth, statecode == "single"), 
